...
export default class User {
  ...
  static saveLocalStore(name, data) {
    try {
  ...
  constructor({ req } = {}) {
    ...
    try {
      if (req) {
        ...
        this.user = User.getLocalStore('user');
      }
    } catch (err) {
      ...
      xhr.onreadystatechange = () => {
        if (xhr.readyState === 4) {
          if (xhr.status === 200) {
            ...
            User.saveLocalStore('user', this.user);
            resolve(this.user);
          } else {
